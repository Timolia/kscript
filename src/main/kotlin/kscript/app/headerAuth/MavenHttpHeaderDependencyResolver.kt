...
class MavenHttpHeaderDependencyResolver : ExternalDependenciesResolver {

    private val originalResolver = OriginalMavenDependenciesResolver()

    override fun acceptsArtifact(artifactCoordinates: String): Boolean
            = originalResolver.acceptsArtifact(artifactCoordinates)

    override fun acceptsRepository(repositoryCoordinates: RepositoryCoordinates): Boolean
            = originalResolver.acceptsRepository(repositoryCoordinates)
    ...
    override suspend fun resolve(
        ...
        try {
            // call new MavenHttpHeaderResolverSession
            val deps = MavenHttpHeaderResolverSession(
                originalResolver.localRepo,
                originalResolver.remoteRepositories()
            ).resolve(artifactId, JavaScopes.RUNTIME)
        ...
        return makeResolveFailureResult(
            originalResolver.allRepositories().map { "$it: $artifactId not found" }, sourceCodeLocation
        )
    }
    ...
    override fun addRepository(
        ...
        // pass auth credentials as HttpHeaderAuthentication
        if (repositoryCoordinates is MavenRepositoryCoordinates) {
            val username = repositoryCoordinates.username?.let(originalResolver::tryResolveEnvironmentVariable)
            val password = repositoryCoordinates.password?.let(originalResolver::tryResolveEnvironmentVariable)

            if (username != null && password != null) {
                repo.setAuthentication(HttpHeaderAuthentication(username, password))
            }
        }

        originalResolver.repos.add(repo.build())
        return true.asSuccess()
    }

    // can't be called in OriginalMavenDependenciesResolver
    fun String.toMavenArtifact(): DefaultArtifact? =
    ...
    /**
     * Original class MavenDependenciesResolver from kotlin.script.experimental.dependencies.maven.impl
     * without private modifier for methods and properties
     */
    private inner class OriginalMavenDependenciesResolver : ExternalDependenciesResolver {
        ...
        fun remoteRepositories() = if (repos.isEmpty()) arrayListOf(mavenCentral) else repos

        fun allRepositories() = remoteRepositories() + localRepo
        ...
        fun tryResolveEnvironmentVariable(str: String) =
